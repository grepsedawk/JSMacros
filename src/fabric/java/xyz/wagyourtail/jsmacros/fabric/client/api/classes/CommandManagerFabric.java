...
public class CommandManagerFabric extends CommandManager {
    ...
    public CommandNodeHelper unregisterCommand(String command) throws IllegalAccessException {
        CommandNode<?> cnf = CommandNodeAccessor.remove(ClientCommands.getActiveDispatcher().getRoot(), command);
        CommandNode<?> cn = null;
    ...
    public void reRegisterCommand(CommandNodeHelper node) {
        if (node.fabric != null) {
            ClientCommands.getActiveDispatcher().getRoot().addChild((CommandNode) node.fabric);
        }
