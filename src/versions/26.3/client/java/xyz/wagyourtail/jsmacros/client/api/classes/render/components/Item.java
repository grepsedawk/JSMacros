...
public class Item implements RenderElement, Alignable<Item> {
    ...
    private static final float OVERLAY_TEXT_Z_OFFSET = 0.02f;
    private static final RenderPipeline OVERLAY_SEE_THROUGH = RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
            .withLocation("pipeline/jsmacros/item_overlay_see_through")
            .withColorTargetState(new ColorTargetState(BlendFunction.TRANSLUCENT))
            .withCull(false)
            .withDepthStencilState(new DepthStencilState(CompareOp.ALWAYS_PASS, false))
            .build();
    private static final RenderPipeline OVERLAY = RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
            .withLocation("pipeline/jsmacros/item_overlay")
            .withColorTargetState(new ColorTargetState(BlendFunction.TRANSLUCENT))
            .withCull(false)
            .build();
    private static final RenderType OVERLAY_SEE_THROUGH_TYPE = RenderType.create(
            "jsmacros_item_overlay_see_through",
            RenderSetup.builder(OVERLAY_SEE_THROUGH).createRenderSetup());
    private static final RenderType OVERLAY_TYPE = RenderType.create(
            "jsmacros_item_overlay",
            RenderSetup.builder(OVERLAY).createRenderSetup());
    ...
    public void render3D(PoseStack matrixStack, int light, boolean seeThrough, SubmitNodeCollector collector, float delta) {
        ...
        if (rotateCenter) {
            ...
            matrixStack.rotate(new Quaternionf().rotateLocalZ((float) Math.toRadians(rotation)));
            matrixStack.translate(-DEFAULT_ITEM_SIZE / 2d, -DEFAULT_ITEM_SIZE / 2d, 0);
        } else {
            matrixStack.rotate(new Quaternionf().rotateLocalZ((float) Math.toRadians(rotation)));
        }
        ...
        try (SurfaceItemSeeThroughScope ignored = SurfaceItemSeeThroughScope.enter(seeThrough)) {
            matrixStack.pushPose();
            matrixStack.translate(DEFAULT_ITEM_SIZE / 2d, DEFAULT_ITEM_SIZE / 2d, 0);
            ...
            if (overlay) {
                ...
                if (item.isBarVisible()) {
                    submitOverlayQuad(matrixStack, collector, seeThrough, 2, 13, 13, 2, 0xFF000000);
                    submitOverlayQuad(matrixStack, collector, seeThrough, 2, 13, item.getBarWidth(), 1,
                            0xFF000000 | item.getBarColor());
                }
                if (mc.player != null) {
                    float cooldown = mc.player.getCooldowns().getCooldownPercent(item, delta);
                    if (cooldown > 0) {
                        int top = Mth.floor(16.0f * (1.0f - cooldown));
                        int bottom = top + Mth.ceil(16.0f * cooldown);
                        submitOverlayQuad(matrixStack, collector, seeThrough, 0, top, 16, bottom - top, 0x7FFFFFFF);
                    }
                }
                matrixStack.popPose();
                ...
                if (text != null) {
                    ...
                    matrixStack.pushPose();
                    matrixStack.translate(0, 0, OVERLAY_TEXT_Z_OFFSET);
                    collector.submitText(matrixStack, tx, ty, Component.literal(text).getVisualOrderText(), true,
    ...
    @DocletIgnore
    public void render3DBack(PoseStack matrixStack, int light, boolean seeThrough, SubmitNodeCollector collector, float delta) {
        if (item == null || !overlay) {
            return;
        }
        String text = ovText != null ? ovText : (item.getCount() > 1 ? String.valueOf(item.getCount()) : null);
        if (text == null) {
        ...
        matrixStack.translate(0, 0, -OVERLAY_TEXT_Z_OFFSET);
        matrixStack.translate(tx + font.width(text) / 2d, ty + font.lineHeight / 2d, 0);
        matrixStack.rotate(new Quaternionf().rotateLocalY((float) Math.PI));
        matrixStack.translate(-tx - font.width(text) / 2d, -ty - font.lineHeight / 2d, 0);
        try (SurfaceItemSeeThroughScope ignored = SurfaceItemSeeThroughScope.enter(seeThrough)) {
            collector.submitText(matrixStack, tx, ty, Component.literal(text).getVisualOrderText(), true,
                    seeThrough ? Font.DisplayMode.SEE_THROUGH : Font.DisplayMode.POLYGON_OFFSET,
    ...
    private static void submitOverlayQuad(PoseStack matrixStack, SubmitNodeCollector collector, boolean seeThrough,
        ...
        RenderType renderType = seeThrough ? OVERLAY_SEE_THROUGH_TYPE : OVERLAY_TYPE;
        collector.submitCustomGeometry(matrixStack, renderType, (pose, vertices) -> {
            vertices.addVertex(pose, x, y, 0).setColor(r, g, b, a);
            vertices.addVertex(pose, x + width, y, 0).setColor(r, g, b, a);
            vertices.addVertex(pose, x + width, y + height, 0).setColor(r, g, b, a);
            vertices.addVertex(pose, x, y + height, 0).setColor(r, g, b, a);
        });
    }
