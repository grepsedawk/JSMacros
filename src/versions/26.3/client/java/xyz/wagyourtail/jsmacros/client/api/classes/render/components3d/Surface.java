...
import xyz.wagyourtail.jsmacros.client.api.classes.render.SurfaceOrderedSubmitNodeCollector;
...
public class Surface extends Draw2D implements RenderElement, RenderElement3D<Surface> {
    ...
    public void render(PoseStack matrices, SubmitNodeCollector collector, float partialTicks) {
        ...
        Pos3D renderPos = isTrackingEntity ? boundEntity.getPos(partialTicks).add(boundOffset) : pos;
        ...
        if (rotateCenter) {
            ...
            matrices.rotate(new Quaternionf().rotateLocalY((float) Math.toRadians(rotations.y)));
            matrices.translate(-sizes.x / 2, 0, 0);
            matrices.translate(0, -sizes.y / 2, 0);
            matrices.rotate(new Quaternionf().rotateLocalX((float) Math.toRadians(rotations.x)));
            matrices.translate(0, sizes.y / 2, 0);
            matrices.translate(sizes.x / 2, -sizes.y / 2, 0);
            matrices.rotate(new Quaternionf().rotateLocalZ((float) Math.toRadians(rotations.z)));
            matrices.translate(-sizes.x / 2, sizes.y / 2, 0);
        } else {
        ...
        if (!renderBack) {
            Vector3f origin = matrices.last().pose().transformPosition(new Vector3f());
            Vector3f xAxis = matrices.last().pose().transformPosition(new Vector3f(1, 0, 0));
            Vector3f yAxis = matrices.last().pose().transformPosition(new Vector3f(0, 1, 0));
            Vector3f normal = yAxis.sub(origin).cross(xAxis.sub(origin));
            Vector3f towardCamera = origin.negate(new Vector3f());
    ...
    private void renderElements3D(PoseStack matrices, SubmitNodeCollector collector, float delta, int light, boolean seeThrough, boolean renderBack, Iterator<RenderElement> iter) {
        renderElements3D(matrices, collector, delta, light, seeThrough, renderBack, iter, 0);
    }

    private int renderElements3D(PoseStack matrices, SubmitNodeCollector collector, float delta, int light, boolean seeThrough, boolean renderBack, Iterator<RenderElement> iter, int sequence) {
        while (iter.hasNext()) {
            ...
            if (element instanceof Draw2DElement draw2DElement) {
                sequence = renderDraw2D3D(matrices, collector, delta, light, seeThrough, renderBack, draw2DElement, sequence);
            } else {
                renderElement3D(matrices, collector, delta, light, seeThrough, renderBack, element, sequence++);
            }
        }
        return sequence;
    }

    private int renderDraw2D3D(PoseStack matrices, SubmitNodeCollector collector, float delta, int light, boolean seeThrough, boolean renderBack, Draw2DElement element, int sequence) {
        matrices.pushPose();
        ...
        matrices.rotate(new Quaternionf().rotateLocalZ((float) Math.toRadians(element.rotation)));
        if (element.rotateCenter) {
            matrices.translate(-element.getWidth() / 2d, -element.getHeight() / 2d, 0);
        }
        ...
        synchronized (draw2D.getElements()) {
            sequence = renderElements3D(matrices, collector, delta, light, seeThrough, renderBack, draw2D.getElementsByZIndex(), sequence);
        }
        matrices.popPose();
        return sequence;
    }

    private void renderElement3D(PoseStack matrices, SubmitNodeCollector collector, float delta, int light, boolean seeThrough, boolean renderBack, RenderElement element, int sequence) {
        matrices.pushPose();
        ...
        SubmitNodeCollector orderedCollector = new SurfaceOrderedSubmitNodeCollector(collector, sequence);
        try (SurfaceElementCompositionScope ignored = SurfaceElementCompositionScope.enter(seeThrough || element instanceof Text, seeThrough)) {
            element.render3D(matrices, light, seeThrough, orderedCollector, delta);
            if (renderBack && element instanceof Text text) {
                text.render3DBack(matrices, light, seeThrough, orderedCollector, delta);
            } else if (renderBack && element instanceof Item item) {
                item.render3DBack(matrices, light, seeThrough, orderedCollector, delta);
            }
        }
        matrices.popPose();
    }
